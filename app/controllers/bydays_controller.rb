...
class BydaysController < ApplicationController
  ...
  def index
    ...
    if params[:list]
      ...
      @fblinks = @fblinks.where('list LIKE ?', listid.to_s)
    end
    @fblinks = @fblinks.where(link_domain: params[:domain]) if params[:domain]

    @fblinks = @fblinks.group(:link_domain).count.sort_by { |_k, v| v }.reverse
  ...
  def indexx
    ...
    if params[:date]
      @byday = @byday.where(created_at: params[:date].to_date.beginning_of_day..params[:date].to_date.end_of_day)
    end
