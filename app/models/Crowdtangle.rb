...
class Crowdtangle < ApplicationRecord
  def self.search(params)
    require 'net/https'
    token = ENV['CT_TOCKEN']
    uri = URI("https://api.crowdtangle.com/posts/search/?token=#{token}&searchTerm=#{URI.escape params[:q]}&startDate=#{params[:start_date]}&endDate=#{(params[:end_date].to_date + 1.day).strftime('%Y-%m-%d')}&sortBy=interaction_rate&count=100")
