...
class Twint < ApplicationRecord
  def self.count_result(params)
    ...
    result = Timeout.timeout(30) { 
      ...
      req.body = {"query":{"bool":{"must":[{"match":{"tweet":"#{params[:q]}"}},{"range":{"date":{"gte":"#{params[:start_date]} 00:00:00","lte":"#{params[:end_date]} 23:59:59"}}}],"must_not":[],"should":[]}},"from":0,"size":100,"sort":[{"date": {"order": "asc"}}],"aggs":{}}.to_json
